...
Extensibility also reaches outward to AI coding assistants: the app spawns and manages a standalone **MCP (Model Context Protocol) server** as a child process, exposing the workflow schema, examples, validation, and a non-destructive back-channel that imports assistant-authored workflows straight into the open project, all behind a loopback + bearer-token + Origin + TLS boundary. See the [MCP server architecture](./MCP.md) and [ADR-0011](../adr/0011-app-managed-mcp-server.md).
